 ...
 * Created by klugemonkey on 11/10/17.
 */
public class v1_8_R1 implements NMS {
    ...
    public String getMinecraftId(ItemStack itemStack) {
        ...
        final MinecraftKey key = (MinecraftKey) Item.REGISTRY.keySet()
                .stream()
                .filter(o -> Item.getId((Item) Item.REGISTRY.get(o)) == id)
                .findFirst()
                .orElse(null);
    ...
    public Material getMaterialFromKey(String key) {
        ...
        ItemStack item = CraftItemStack.asNewCraftStack((Item) Item.REGISTRY.get(minecraftKey));
        return item.getType();
    }
